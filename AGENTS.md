...
- **`src/index.ts`** — `createEnvoker(schema, options?)` → returns `Envoker` with `ensure()`, `get()`, `isEnabled()`, `isKilled()`.
- **`src/types.ts`** — `FieldDefinition`, `SchemaEntry`, `EnvokerOptions`, `FeatureGate`, `EnsureOptions`.
...
- **`src/cli.ts`** — CLI entry point (`envoker`); parses args, dispatches to handlers.
...
- **Named exports only** (no `export default` except `index.ts` which defaults to `createEnvoker`).
